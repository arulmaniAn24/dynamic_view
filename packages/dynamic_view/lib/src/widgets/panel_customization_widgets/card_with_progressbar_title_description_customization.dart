...
class CardWithProgressBarTitleDescriptionCustomization extends StatefulWidget {
  final WidgetModel? widget;

  const CardWithProgressBarTitleDescriptionCustomization({
    required this.widget,
    super.key,
  });

  @override
  State<CardWithProgressBarTitleDescriptionCustomization> createState() =>
      _CardWithProgressBarTitleDescriptionCustomizationState();
}

class _CardWithProgressBarTitleDescriptionCustomizationState
    extends State<CardWithProgressBarTitleDescriptionCustomization> {
  ...
  void didUpdateWidget(
      covariant CardWithProgressBarTitleDescriptionCustomization oldWidget) {
  ...
  void _initControllers() {
    ...
    _labelFontSizeController = TextEditingController(
        text: widget.widget?.properties['titleFontSize']?.toString() ?? '18');
    _valueFontSizeController = TextEditingController(
        text: widget.widget?.properties['valueFontSize']?.toString() ?? '24');
    _descriptionFontSizeController = TextEditingController(
        text: widget.widget?.properties['descriptionFontSize']?.toString() ??
            '14');
    _widthController = TextEditingController(
        text: widget.widget?.properties['width']?.toString() ?? '300');
    _heightController = TextEditingController(
        text: widget.widget?.properties['height']?.toString() ?? '100');
    ...
    _paddingDyController = TextEditingController(
        text: widget.widget?.properties['paddingDy']?.toString() ?? '8');
  ...
  void _updateControllers() {
    _widthController.text =
        widget.widget?.properties['width']?.toString() ?? '300';
    _heightController.text =
        widget.widget?.properties['height']?.toString() ?? '100';
  }
    ...
    return BlocBuilder<ViewBuilderBloc, ViewBuilderState>(
      builder: (context, state) {
        ...
        Widget buildFieldsForKey(String key) {
          switch (key) {
            case 'cardWithProgressbarTitleDescriptiontitle':
              return Column(
            ...
            case 'cardWithProgressbarTitleDescriptionvalue':
              return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Gap(10),
            ...
            case 'cardWithProgressbarTitleDescriptiondescription':
              return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  CustomValueTextfield(
